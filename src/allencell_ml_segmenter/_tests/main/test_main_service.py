...
def test_service_sets_channels_on_exp_applied_mixed():
    ...
    exp_model: FakeExperimentsModel = FakeExperimentsModel(channel_selection_path=mixed_channel_path)
    service: MainService = MainService(main_model, exp_model, task_executor=SynchroTaskExecutor.global_instance(), file_writer=FakeFileWriter())

    # Assert (sanity check)
    channels: dict[ImageType, Optional[int]] = main_model.get_selected_channels()
...
def test_service_does_not_set_channels_on_exp_applied_nonexistent():
    # Arrange
    main_model: MainModel = MainModel()
    exp_model: FakeExperimentsModel = FakeExperimentsModel(channel_selection_path=nonexistent_channel_path)
    service: MainService = MainService(main_model, exp_model, task_executor=SynchroTaskExecutor.global_instance(), file_writer=FakeFileWriter())
...
def test_service_writes_to_disk_when_channels_change():
    ...
    service: MainService = MainService(main_model, exp_model, task_executor=SynchroTaskExecutor.global_instance(), file_writer=file_writer)
    ...
    assert file_writer.json_state[nonexistent_channel_path] == {"raw": 8, "seg1": 2, "seg2": 7}