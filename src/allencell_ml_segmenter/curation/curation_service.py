...
class CurationService(QObject):
    ...
    def _save_curation_record(
        ...
        FileUtils.write_curation_record(
            curation_record, csv_path, mask_dir_path
        )
    ...
    def _on_save_to_disk(self) -> None:
        ...
        record_saver: FunctionWorker = self._save_curation_record(
            curation_record,
            self._curation_model.get_csv_path(),
            self._curation_model.get_save_masks_path(),
        )
        record_saver.finished.connect(
            lambda: self._curation_model.set_curation_record_saved_to_disk(
                True
            )
        )
        record_saver.start()
